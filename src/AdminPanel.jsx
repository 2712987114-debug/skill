...
export default function AdminPanel({ content, onContentChange }) {
  ...
  const [aboutPrimary, setAboutPrimary] = useState(() => content.profile?.aboutPrimary || "我叫 Cui Mengyuan，是一名专注内容叙事与视觉表达的 AI 短剧剪辑师。熟悉真人短剧的粗剪、精剪与节奏把控，也能独立完成 AI 漫剧从小说改写、剧本分镜、资产图建立、视频生成到剪辑成片的完整流程。");
  ...
  const [displayName, setDisplayName] = useState(() => content.profile?.displayName || "Cui Mengyuan");
  const [heroRole, setHeroRole] = useState(() => content.profile?.heroRole || "剪辑师 / AI设计师 / AI漫剧");
  const [footerCopyright, setFooterCopyright] = useState(() => content.profile?.footerCopyright || "© 2024—2026 Cui Mengyuan · 保留所有权利");
  ...
  useEffect(() => {
    ...
    setAboutPrimary(profile.aboutPrimary || "我叫 Cui Mengyuan，是一名专注内容叙事与视觉表达的 AI 短剧剪辑师。熟悉真人短剧的粗剪、精剪与节奏把控，也能独立完成 AI 漫剧从小说改写、剧本分镜、资产图建立、视频生成到剪辑成片的完整流程。");
    ...
    setDisplayName(profile.displayName || "Cui Mengyuan");
    setHeroRole(profile.heroRole || "剪辑师 / AI设计师 / AI漫剧");
    setFooterCopyright(profile.footerCopyright || "© 2024—2026 Cui Mengyuan · 保留所有权利");
  }, [content.profile]);
